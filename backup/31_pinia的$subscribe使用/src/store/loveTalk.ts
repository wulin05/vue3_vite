...
export const useTalkStore = defineStore('talk', {
  state() {
    return {
      ...
      // ##### 可以不用再上面设置默认的talkList, 通过读取LoveTalk.vue中的talkStore.$subscribe函数将数据存储到浏览器,并且转换为JSON对象
      // as string 作为 localStorage.getItem('talkList') 的断言; 并且为了更完善,还需要当前面为null的时候,talkList=[]空数组
